...
def contact(request):
    if request.method == "POST":
        ...
        data = Contact(name=name, email=email,subject=subject,message=message, datetime=datetime.now())
        data.save()
...
def show_enquiries(request):
    ...
    dt = Contact.objects.order_by('-id')
    count = dt.count()
    data = {"dt":dt, "count":count}
    return render(request,'enquiries.html', data)
